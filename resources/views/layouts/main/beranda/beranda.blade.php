    ...
    <div class="container">
        ...
        <div class="container mt-5 mb-5">
            ...
            <div class="container mt-5 mb-5"> <!-- Search bar and filter -->
                <div class="row mb-3">
                    <div class="col-md-6"> <input type="text" id="search" class="form-control" placeholder="Search products..."> </div>
                    <div class="col-md-6"> <select id="filterType" class="form-select">
                            <option value="">All Types</option> @foreach ($types as $type) <option value="{{ $type }}">{{ $type }}</option> @endforeach
                        </select> </div>
                </div>
                <div class="row g-1" id="productContainer">
                        ...
                        <div class="p-card">
                                ...
                                <div id="carousel-{{ $produk->id }}" class="carousel slide" data-bs-ride="carousel">
                                    <div class="carousel-inner">
                                        <div class="carousel-item active">
                                            <img src="{{ asset('storage/' . $produk->photo_main) }}" class="w-100 d-block" alt="{{ $produk->name }}">
                                        </div>
                                        @if ($produk->photo_1)
                                        <div class="carousel-item">
                                            <img src="{{ asset('storage/' . $produk->photo_1) }}" class="w-100 d-block" alt="{{ $produk->name }}">
                                        </div>
                                        @endif
                                        @if ($produk->photo_2)
                                        <div class="carousel-item">
                                            <img src="{{ asset('storage/' . $produk->photo_2) }}" class="w-100 d-block" alt="{{ $produk->name }}">
                                        </div>
                                        @endif
                                        @if ($produk->photo_3)
                                        <div class="carousel-item">
                                            <img src="{{ asset('storage/' . $produk->photo_3) }}" class="w-100 d-block" alt="{{ $produk->name }}">
                                        </div>
                                        @endif
                                        @if ($produk->photo_4)
                                        <div class="carousel-item">
                                            <img src="{{ asset('storage/' . $produk->photo_4) }}" class="w-100 d-block" alt="{{ $produk->name }}">
                                        </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="carousel-indicators">
                                        <button type="button" data-bs-target="#carousel-{{ $produk->id }}" data-bs-slide-to="0" class="active" aria-current="true"></button>
                                        @if ($produk->photo_1)
                                        <button type="button" data-bs-target="#carousel-{{ $produk->id }}" data-bs-slide-to="1"></button>
                                        @endif
                                        @if ($produk->photo_2)
                                        <button type="button" data-bs-target="#carousel-{{ $produk->id }}" data-bs-slide-to="2"></button>
                                        @endif
                                        @if ($produk->photo_3)
                                        <button type="button" data-bs-target="#carousel-{{ $produk->id }}" data-bs-slide-to="3"></button>
                                        @endif
                                        @if ($produk->photo_4)
                                        <button type="button" data-bs-target="#carousel-{{ $produk->id }}" data-bs-slide-to="4"></button>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="p-details">
                                <div class="d-flex justify-content-between align-items-center mx-2">
                                    <h5>{{ $produk->name }}</h5><span>Rp {{ number_format($produk->price, 0, ',', '.') }}</span>
                                </div>
                                <div class="mx-2">
                                    <hr class="line">
                                </div>
                                <div class="d-flex justify-content-between mt-2 spec mx-2">
                                    ...
                                    <div class="d-flex flex-column align-items-center">
                                        <h6 class="mb-0">STOCK</h6><span>{{ $produk->stock }}</span>
                                    </div>
                                    <div class="d-flex flex-column align-items-center">
                                ...
                                <div class="buy mt-3"><button class="btn btn-primary btn-block" type="button" data-bs-toggle="modal" data-bs-target="#buyNowModal-{{ $produk->id }}">Buy Now</button></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="buyNowModal-{{ $produk->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="buyNowModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-body">
                                    <div class="row">
                                        <div class="col-md-6 text-center mt-2">
                                            <img id="activeImage-{{ $produk->id }}" src="{{ asset('storage/' . $produk->photo_main) }}" width="300">
                                            <div class="mt-3">
                                                <div class="thumbnail active" id="thumbnail-{{ $produk->id }}-main" onclick="changeImage('{{ asset('storage/' . $produk->photo_main) }}', {{ $produk->id }}, 'main')">
                                                    <img src="{{ asset('storage/' . $produk->photo_main) }}" width="50">
                                                    <div class="overlay"></div>
                                                </div>
                                                @if($produk->photo_1)
                                                <div class="thumbnail" id="thumbnail-{{ $produk->id }}-1" onclick="changeImage('{{ asset('storage/' . $produk->photo_1) }}', {{ $produk->id }}, '1')">
                                                    <img src="{{ asset('storage/' . $produk->photo_1) }}" width="50">
                                                    <div class="overlay"></div>
                                                </div>
                                                @endif
                                                @if($produk->photo_2)
                                                <div class="thumbnail" id="thumbnail-{{ $produk->id }}-2" onclick="changeImage('{{ asset('storage/' . $produk->photo_2) }}', {{ $produk->id }}, '2')">
                                                    <img src="{{ asset('storage/' . $produk->photo_2) }}" width="50">
                                                    <div class="overlay"></div>
                                                </div>
                                                @endif
                                                @if($produk->photo_3)
                                                <div class="thumbnail" id="thumbnail-{{ $produk->id }}-3" onclick="changeImage('{{ asset('storage/' . $produk->photo_3) }}', {{ $produk->id }}, '3')">
                                                    <img src="{{ asset('storage/' . $produk->photo_3) }}" width="50">
                                                    <div class="overlay"></div>
                                                </div>
                                                @endif
                                                @if($produk->photo_4)
                                                <div class="thumbnail" id="thumbnail-{{ $produk->id }}-4" onclick="changeImage('{{ asset('storage/' . $produk->photo_4) }}', {{ $produk->id }}, '4')">
                                                    <img src="{{ asset('storage/' . $produk->photo_4) }}" width="50">
                                                    <div class="overlay"></div>
                                                </div>
                                                @endif
                                            </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="text-white mt-5">
                                                ...
                                                <span class="intro-1">Harga: Rp {{ number_format($produk->price, 0, ',', '.') }}</span>
                                                <div class="mt-4">
                                                    <span class="intro-2">Stok: {{ $produk->stock }}</span>
                                                </div>
                                                <div class="mt-4">
                                                    <span class="intro-2">{{ $produk->description }}</span>
                                                </div>
                                                <div class="mt-4 mb-5">
                                                    <button class="btn btn-primary">Confirm Purchase <i class="fa fa-long-arrow-right"></i></button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
